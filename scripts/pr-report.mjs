...
function sizeTable(headSize, baseSize) {
    ...
    // Union of both sides' files, in a stable order (head's own order first,
    // then any base-only entries) - a file the base branch still has but the
    // head branch removed (or renamed) should still show up as a row.
    const files = [
        ...(headSize ?? []).map((r) => r.file),
        ...Object.keys(baseByFile).filter((f) => !(f in headByFile)),
    ];
