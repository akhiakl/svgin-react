...
Fetch and cache an SVG ahead of time, so it renders instantly later:
...
- `svgin-react` or `svgin-react/client`: the client component only.
...
CI runs all of the above (across Node 18/20/22 for tests) on every pull request and push to `main`. Releases are published to npm from `.github/workflows/release.yml` when a GitHub Release is published, after re-running the full lint/typecheck/test/build/size gate against the tagged commit.
