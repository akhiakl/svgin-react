...
- [`uniquifyIds`](src/utils/svgUtils.ts) (the automatic id-collision handling mentioned above) only rewrites `id="..."` attributes and references to them via `url(#id)` or `href="#id"` - it does not rewrite CSS selectors inside a `<style>` block (e.g. `#icon-a path { ... }`). Rendering the same SVG twice still gets each instance its own unique `id`s on the elements themselves, but a `<style>` rule that targets one of those ids by selector keeps targeting the *original*, un-suffixed id - which no longer exists on either rendered copy - so the rule silently stops matching anything.
