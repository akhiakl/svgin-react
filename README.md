...
- [`uniquifyIds`](src/utils/svgUtils.ts) (the automatic id-collision handling mentioned above) only rewrites `id="..."` attributes inside the SVG's *inner* markup (its child elements - it's never applied to the outer `<svg>` element itself, which React renders and only forwards source attributes onto) and references to those ids via `url(#id)`, `href="#id"`, or `xlink:href="#id"` - it does not rewrite CSS selectors inside a `<style>` block (e.g. `#icon-a path { ... }`). Rendering the same SVG twice still gets each instance's inner elements their own unique `id`s, but a `<style>` rule that targets one of those ids by selector keeps targeting the *original*, un-suffixed id - which no longer exists on either rendered copy - so the rule silently stops matching anything.
